...
namespace sidebartest
{
    public partial class FormQuanLyDonHang : Form
    {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                else if (btn_hoantat.Text == "Hoàn Tất")
                {
                    ...
                    switch (getuser)
                    {
                        ...
                        case "existGH":
                            {
                                MessageBox.Show("ĐÃ CÓ NHÂN VIÊN GIAO HÀNG NHẬN!!!", "THÔNG BÁO!!!", MessageBoxButtons.OK, MessageBoxIcon.Error);
                                break;
                            }
                        default:
                            {
        ...
        {
            dataTable.Clear();
            string query = "SELECT Ma_Don_Hang N'Mã_Đơn_Hàng', Thanh_Tien N'Thành_Tiền', Ngay_Xuat_Don N'Ngày_Xuất_Đơn', IIF(Trang_Thai_Don = 1, N'Hoàn Tất', N'Chưa Hoàn Tất') AS N'Trạng_Thái_Đơn', Ma_Khach_Hang N'Mã_Khách_Hàng', Ma_Nhan_Vien N'Mã_Nhân_Viên' FROM dbo.DonHang";
